...
d0=2.0e-3;                   %光斑直径:m
for crstl_L=(44.5:5:120)*1e-3           %晶体长度:m
z1=0;                        %积分起点
z2=crstl_L;                  %积分终点:m
s=2;                         %窗口宽度因子
%初始电场强度
...
saveas(gcf, ['data\crstl_L=',num2str(crstl_L),'.bmp'], 'bmp'); 
%画出参量作用后波前变化
figure(2)
subplot(2,2,1)
title('空间强度调制');
...
E(:,:)=E_P_ph(nstep,:,:);
mesh(X*1e3,Y*1e3,E.*W_F)
saveas(gcf, ['data\wavefront_crstl_L=',num2str(crstl_L),'.bmp'], 'bmp'); 
toc    
close all
end